...
- 2026-09-25 10:54 — probe ke **Wasabi asli** (bucket `task-monitor`, region `ap-southeast-1`, sub-user khusus bucket). Dijalankan dari i5 lewat kode driver `s3` yang sama persis dengan yang dipakai app; kredensial dibaca skrip dari file config di luar repo, di dalam prosesnya, output disaring (nilai kunci tidak pernah tercetak; skrip dihapus setelahnya). 7 pengecekan lulus, 0 gagal: (AC 7) PUT diterima dengan **checksum bawaan SDK**, jadi `S3_CHECKSUM` cukup `default`; (AC 2, level driver) PUT lalu GET balik 70 byte dan 5 MB, isi sama persis; (asumsi ListBucket) key yang gak ada dijawab **404 NoSuchKey**, driver balikin `null`, jadi lampiran hilang = 404 rapi; (AC 8) secret salah ditolak `SignatureDoesNotMatch` 403, bucket asing `NoSuchBucket` 404; hapus lewat driver bekerja (GET setelahnya `null`); (cakupan policy) PUT di luar prefix `storage/attachment/` ditolak `AccessDenied`, `ListAllMyBuckets` ditolak, jadi kunci hanya bisa menyentuh folder lampiran di bucket itu. Semua objek uji (prefix `tmprobe-`) sudah dihapus, sisa 0. Objek uji dihapus <90 hari, jadi kena tagihan sisa hari minimum storage duration (beberapa MB, nilainya kecil). **Sisa**: uji ujung-ke-ujung lewat app (form task baru, baris DB, halaman detail) dengan kunci Wasabi asli; AC 3 (cleanup kalau transaksi gagal) sudah terbukti lawan S3 palsu dan DeleteObject di Wasabi terbukti bekerja, tapi belum lewat alur app + Wasabi.
- 2026-09-25 11:39 — beres, dipindah ke done. Uji ujung-ke-ujung lewat app dengan Wasabi asli dilakukan Iyan sendiri (form Task baru, lampiran tersimpan di bucket, tampil di halaman detail) dan lulus per laporan Iyan; lampiran hasil tes itu ada di bucket (5 objek). Sisa objek uji dari probe (`tes-manual-2026-09-25/tes-upload.png`) dihapus lewat driver. Ikut berubah di sesi yang sama: tombol hapus lampiran satu-satu di form Task baru, modal viewer lampiran (zoom, unduh) di halaman detail, dan favicon disamain dengan primary baru (`#2A7C13`).
